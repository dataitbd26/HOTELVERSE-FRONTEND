...
import { 
  ...
  FaChevronLeft, FaChevronRight, FaWrench, FaTags,
  FaCheck, FaBan
} from "react-icons/fa";
...
const WorkOrderCategoryManager = () => {
  const { user } = useAuth();
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      const data = await getAllWorkOrderCategories({ 
        page: currentPage, 
        limit: itemsPerPage, 
      ...
      toast.error("Failed to load work order categories");
    }
  }, [getAllWorkOrderCategories, currentPage, itemsPerPage, debouncedSearch, userBranch]);
  ...
  // --- Handlers ---
  const handleOpenForm = useCallback((category = null) => {
    setErrors({}); 
    if (category) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updateWorkOrderCategory(editingId, payload);
        toast.success("Category updated successfully");
      } else {
        await createWorkOrderCategory(payload);
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await removeWorkOrderCategory(id);
  ...
  }, [removeWorkOrderCategory, loadCategories, categoriesList.length, currentPage]);
  ...
  return (
    <div className="bg-[#f1f5f9] dark:bg-gray-900 min-h-screen p-4 md:p-6 text-[#1f2937] dark:text-gray-100 transition-colors duration-300 relative overflow-hidden font-sans">
      ...
      <Helmet>
        <title>Work Order Categories | Admin Dashboard</title>
        <meta name="description" content="Manage work order classifications and categories." />
      </Helmet>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <PageHeader 
          ...
          subtitle="Manage work order classifications" 
        ...
        <div className="p-5 border-b border-slate-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50 dark:bg-gray-800/50">
          ...
          <div className="flex items-center gap-4 flex-wrap sm:flex-nowrap w-full sm:w-auto">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <label htmlFor="searchCategory" className="text-sm font-bold hidden sm:block">Search:</label>
              <input 
                id="searchCategory"
                type="search"
        ...
        <div className="overflow-x-auto p-5 min-h-[400px]">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-gray-700 border-x border-b border-slate-200 dark:border-gray-700">
              ...
              {loading ? (
              ...
              ) : categoriesList.length === 0 && !isFormOpen ? (
                <tr>
                ...
                categoriesList.map((category) => (
                  <tr key={category._id} className="hover:bg-slate-50 dark:hover:bg-gray-700/20 transition-colors">
...
export default WorkOrderCategoryManager;